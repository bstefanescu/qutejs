...
import myCss from './my-component.css';
...
```
You may notice the `Qute.css(myCss);` line. The `CSS` file when imported is returned as a string.  \
You can do then whathever you want to do with. **Qute**, provides you a method to inject this code inside the target HTML page (the one which will import the component) by using `Qute.css(cssCode)`.

When using inline styles through the `<q:style>...</q:style>` tag, the `Qute.css(...)` line is automatically generated by the compiler.
